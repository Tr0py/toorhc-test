...
void __attribute__((section("iv_stub_"STR(name)))) __attribute__((aligned(4096), naked)) CONCAT2(_xcall_stub_, name)(){ \
    asm(".fill 8192, 1, 0xcc"); \
} \
struct { \
...
static unsigned long __attribute__((section("iv_data_"STR(ISONAME)))) __attribute__((aligned(4096), used)) CONCAT2(iv_data_placeholder_, name) = {0}; 

#define INIT_CODESTUB(name) \
static void __attribute__((section("iv_code_"STR(ISONAME)))) __attribute__((aligned(4096), used, naked)) CONCAT2(iv_func_placeholder_, name)(){};
